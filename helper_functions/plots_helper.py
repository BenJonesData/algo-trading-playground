...
def create_candlestick(data: pd.DataFrame) -> go.Candlestick:
    ...
    Args:
        - data (pd.DataFrame): The input DataFrame. The DataFrame must meet the
        following requirements:
            - It must be indexed by a `DatetimeIndex` (i.e., the index must be
            of type `datetime`).
    ...
    Returns:
        go.Candlestick: A Plotly Candlestick object representing the financial
        data.
...
def plot_position(
    ...
    """
    Plots asset price and position size over time using a dual-axis plot, where
    the primary axis shows the asset price and the secondary axis shows the
    position size.

    Args:
        - price_data (pd.DataFrame): A time series DataFrame containing the
        asset price, indexed by a `DatetimeIndex`.
        - position_data (pd.DataFrame): A time series DataFrame containing the
        position size, indexed by a `DatetimeIndex`.
        - asset_label (str): A string representing the asset's name, which is
        used in the plot's title.

    Raises:
        ValueError: If either `price_data` or `position_data` is not indexed by
        a `DatetimeIndex`.

    Notes:
        - The function creates a plot where the asset price is plotted on the
    ...
    if not (
        ...
        raise ValueError(
            "Both `price_data` and `position_data` must be indexed by a"
            "`DatetimeIndex`."
        )
